...
void initializeSensor() {
    ...
    DDRD &= ~((1 << ECHO_PIN1) | (1 << ECHO_PIN2) | (1 << ECHO_PIN3)); // Set Echo pins 1-3 as input
    DDRB &= ~(1 << DDB4); // Set Echo pin 4 (D12/PB4) as input
}
...
long getduration(uint8_t echoPin) {
    ...
    // Handle Port B for ECHO_PIN4
    if (echoPin == ECHO_PIN4) {
        while(!(PINB & (1 << 4))) { // Adjust for PINB and PB4
            if (count++ > maxCount) {
